...
module.exports = {
  handler: function(req, res) {
    ...
    const signed_params = crypto
      .createHmac('sha256', secret)
      .update(body)
      .digest('hex');
    const signed_date = crypto
      .createHmac('sha256', signed_params)
    ...
    //build fetch request
    fetch(
      config[process.env.OD_API_ENV].odrs + req.path.replace(/^\/api\//i, ''),
      {
        method: req.method,
        headers: {
      ...
      .then(data => {
        if (data.status === 'OK') {
          mailer.notify(
            req,
            res,
            getNotificationType(req.path.replace(/^\/api\//i, ''))
          );
        }
        res.json(data);
      });
  ...
  page: function(req, res) {
    ...
    const page_name =
      req.params.page_name && req.params.page_name.length
    ...
    if (req.params && page_name && pageMap[page_name]) {
      ...
      sheetsReader.getTabs().then(tabs => {
        return Promise.all(
          tabs.map((tab, index) => {
            let tabData = {
