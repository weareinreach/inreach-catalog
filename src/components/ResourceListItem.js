          ...
          <Grid item xs={12} >
            <Grid container alignItems="center" justify="space-between" spacing={0}>
              <Grid item xs md lg xl >
                <Link to={'/resource/'+resource.organization.slug}><Typography type="subheading" >{resource.organization.name}</Typography></Link>
              </Grid>
              {format === 'search' ? 
              <Grid item xs={3}>
            ...
            {resourceFieldsByFormat[format].map((item) => {
              var Content;
              switch(format) {
                case 'search':
                  Content = () => (<Typography type="body2">
                    {resource.organization[item.fieldName]}
                  </Typography>);
                break;
                default:
                  Content = () => (<Typography type="body2">
                    <strong>{item.label}:</strong> {resource.organization[item.fieldName]}
                  </Typography>);
                break;
              }
              return (
                <Grid item xs={12} key={resource.organization.id} >
                  <Content />
                </Grid>);
            })}
          <Grid item xs={12} >
            <Grid container alignItems="center" spacing={0} justify="space-between">
              ...
              <Grid item xs={6} className={rightSide}>
                <RatingControl rating={resource.organization.rating} />
                <ReviewCount total={3} />
              </Grid>
            </Grid>
          </Grid>
