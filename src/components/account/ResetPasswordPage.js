...
class ResetPasswordPage extends React.Component {
  ...
  componentWillMount(){
    const {handleMessageNew, handleLogOut, session, location, history} = this.props;
    if(session) {
      handleLogOut();
    }
  ...
  handleSubmit(event) {
    ...
    resetPassword(payload)
      ...
      .catch((error) => {
        handleMessageNew('Failed to reset password, please request a new reset link.');
      });
  ...
  render() {
    ...
    return (
      <Grid container spacing={0} justify="center" alignItems="center" className={classes.container}>
        { !submitted
        ?
        <Grid item xs={11} sm={8} md={5} >
          <form onSubmit={this.handleSubmit}>
            ...
            <TextField
              ...
              helperText={password.length >= 8 && password !== confirmPassword ? 'Confirmation does not match password.' : null}
              id="confirmPassword"
